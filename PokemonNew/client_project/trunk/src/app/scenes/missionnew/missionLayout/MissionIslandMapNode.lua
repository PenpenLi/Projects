...
function MissionIslandMapNode:render()
    ...
    self._city:removeFromParent()
    local bg = panel:getChildByName("Image_1")
    self._infoHolder = panel:getChildByName("Node_info")
    self._infoHolder:retain()
    self._infoHolder:removeFromParent()
    self.nodeTop = panel:getChildByName("Node_top")
	self.nodeTop:addChild(self._city)
	self.nodeTop:addChild(self._infoHolder)
    self._infoHolder:release()
