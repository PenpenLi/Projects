...
function ActivityData:getTypeList()
    ...
    --if G_Me.userData.order == 0 then
        --print("ddddd:G_Me.userData.order2", G_Me.vipData:getExp(), G_Me.shopData:firstRechargeForActivity())
        --if G_Me.vipData:getExp() == 0 or (not G_Me.shopData:firstRechargeForActivity()) then
        if (not hasOrder()) or (not G_Me.shopData:firstRechargeForActivity()) then
            --未充值或者未领取奖励
            table.insert(self._typeList,{
                    id = "shou_chong",
                    ...
                    needShowTip = function()
                        ...
                        return  hasOrder() and not G_Me.shopData:firstRechargeForActivity() 
                    end  
                })
        end
    end
...
function ActivityData:hasActivityToJoin()
    ...
    if G_Setting:get("open_recharge") == "1" then
        ...
        --充值了但是未领取
        if hasOrder() and not G_Me.shopData:firstRechargeForActivity() then
            return true
        end
    end
...
function hasOrder()
    print("hasOrder",G_Me.vipData:getExp(),G_Me.userData.order)  
    return G_Me.vipData:getExp() > 0 and G_Me.userData.order > 0
end
