...
function UserData:setBaseData(user)
    ...
    if self.lastData == nil then
        ...
        self.lastData.order = user.order_num
    else
        if user.money ~= self.money then
            self.lastData.money = self.money
        end

        if user.order_num ~= self.order then
            self.lastData.order = self.order
        end
    ...
    if IS_TASTE_VERSION == false and IS_IPHON_ANDROID == true then 
        TDGAAccount:setLevel(user.level)
        print("统计->设置玩家等级:" .. user.level)
    end
    
    self.vit = user.vit       -- 体力
    self.spirit = user.spirit -- 精力
    self.order = user.order_num
