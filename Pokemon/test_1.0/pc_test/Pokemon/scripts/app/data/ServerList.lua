...
function ServerList:init()
    if not self._inited then
        track_event_start("6-start_get_serverlist")
        self._inited = true
        --[[
        local useDefaultServerList = false
        --local request=uf_netManager:createHTTPRequestGet("http://test.mobile-ease.com:8087/serverlist",function(event)
        --local url = "http://120.26.14.132:8087/serverlist?cv=1.1&opid=" .. ComSdkUtils.getOpId();
        local url = GAME_URL .. "/serverlist?opid=" .. ComSdkUtils.getOpId() .. "&sdk_id=" .. ComSdkUtils.getSdkId()
        url = url .. "&cv=" .. GAME_VERSION_NO
        print("get serverList:", url)
        local request=uf_netManager:createHTTPRequestGet(url,function(event)
            ...
            self._list=json.decode(tables["data"])
            --self:getFirstServer()
            --[[self._list =  {
        ...
        request:start()
            ...
            --    self._list = {}
        self._list =  {
            {name="", id=9, login="http://192.168.2.102/web/hook_login/login.php", status=1, gateway="120.26.205.212",port=8118},}
        ...
        if t then
            ...
            if ok then
                ...
                --if t.status == "1" then
                    ...
                    self._list = list
                    uf_eventManager:dispatchEvent(G_EVENTMSGID.EVENT_UPDATE_SERVER_LIST, nil, true)
                    track_event_start("7-complete_get_serverlist")
                    if callback then
                        callback()
                    end
