...
function ShopDropMainLayer:_initScrollView()
    ...
    if G_Me.userData.vip < 16 then
        ...
        table.insert(widgetList,1,self:getButtonByName("Button_jipin20"))
    end
    print("widgetNum1",widgetNum)
    ...
    if G_Me.userData.level >= 40 then
        ...
        print("x,y", winSize.width, winSize.height)
        self._zhenyingButtonList[1]:setPositionX(x + 20)
        if winSize.height > 960 then
            self._zhenyingButtonList[1]:setPositionY(y - 80)
        else 
    ...
    local scrollviewWith =  widgetNum*WIDTH
    
    --内容高度小于scrollview高度时
    if scrollviewWith < size.width then
        scrollviewWith = size.width
    end
        
    scrollview:setInnerContainerSize(CCSizeMake(scrollviewWith,size.height))
    --重新定位,从顶部开始放,因为2个item的时候必须这么做
    for i=1,#widgetList do
        local widget = widgetList[i]
        local x = scrollviewWith - (i-1/2)*WIDTH
        local y = size.height/2
        print("size.height2", size.height)
        if #widgetList == 2 and widgetNum == 2 then
            if i == 1 then
                widget:setPositionX(x -60)
                widget:setPositionY(y + 100)
            else
                widget:setPositionX(x - 120)
                widget:setPositionY(y + 100)
            end
        else
            widget:setPositionX(x - 220)
            local b = self._zhenyingButtonList[1]:getPositionY()
            widget:setPositionY(b + 130)
        end
