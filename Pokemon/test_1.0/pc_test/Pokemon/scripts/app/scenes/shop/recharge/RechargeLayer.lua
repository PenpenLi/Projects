...
function RechargeLayer:_initListView()
	...
	self._listView:setUpdateCellHandler(function(list,index,cell)
		...
		cell:setRechargeFunc(function()
		    ...
		    local payExtra = "0_0_" .. data.data.app_id
...
function RechargeLayer:_callRecharge(price,productId,productName,payExtra)
	...
	table.insert(_t,{orderId = orderId})
	table.insert(_t,{roleName = G_Me.userData.name})
	table.insert(_t,{roleId = tostring(uid)})
	table.insert(_t,{roleLevel = tostring(G_Me.userData.level)})
	table.insert(_t,{payExtra = ""})
	table.insert(_t,{serverId = tostring(sid)})
	table.insert(_t,{serverName = server.name})

    print("charge:", GlobalFunc.sz_T2S(_t)) 

    function callPay(transactionId, productId, oExInfo)
	    table.insert(_t,{transactionId = transactionId})
        _t[2] = {productId = tostring(productId)}
        _t[12] = {payExtra = tostring(oExInfo)} 
        print("charge2:", GlobalFunc.sz_T2S(_t))
        ComSdkUtils.call("pay",_t)
        --haima支付请求
        if IS_IPHON_ANDROID then
            TDGAVirtualCurrency:onChargeRequst(orderId, productId, price, "CNY", pointRate * price, "游戏支付")
            print("统计->支付订单[" .. orderId .. "], 商品编号[" .. productId .. "], 支付金额[" .. price .. "], 货币[CNY], 虚拟币[" .. pointRate * price .. "], 平台[海马]" )
        end
    end
    --payExtra = GlobalFunc.url_encode(payExtra)
	if IS_APPSTORE_VERSION == false then
		print("ComSdkUtils false")
		self:getOrderId(sdkUid, orderId, payExtra, callPay)
	else
		print("ComSdkUtils true")
		_t[2] = {productId = tostring(productId)}
		print("charge2:", GlobalFunc.sz_T2S(_t))
		ComSdkUtils.call("pay",_t)
	end
...
function RechargeLayer:getOrderId(sdkUid, orderId, exInfo, callback)
    G_WaitingLayer:show(true)
    local url = GAME_URL .. "/get_istorm_order?"
    url = url .. "orderId=" .. orderId
    url = url .. "&sdk_uid=" .. sdkUid 
    --url = url .. "&channel=" .. "1" 
    --url = url .. "&channel=" .. "500100001" 
    url = url .. "&channel=" .. G_NativeProxy.channelId
    url = url .. "&subchannel=" .. G_NativeProxy.subChannelId
    --url = url .. "&exInfo=" .. exInfo
    print("getOdrderId:", url)
    local request = uf_netManager:createHTTPRequestGet(url, function(event)
        ...
        print("getTransactionId:" , response)
        local t=json.decode(response)
        if t == nil or t["Code"] ~= "200" then
            MessageBoxEx.showOkMessage(nil,
                "服务器错误:生成订单失败", false,
                function ( ... )
                end
        ...
        if (callback ~= nil and t["Data"] ~= nil) then
            local data = t["Data"]
            local exInfo = data["oExInfo"]
            exInfo = exInfo and exInfo or ""
            callback(data["transactionId"], data["productId"], exInfo)
        end
