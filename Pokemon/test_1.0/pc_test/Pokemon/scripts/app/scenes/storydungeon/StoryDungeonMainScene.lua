...
function StoryDungeonMainScene:initListView()
    if self.listview == nil then
        ...
        local lenth = math.floor(#self._DungeonList/5) 
        if #self._DungeonList%5 > 0 then
            lenth = lenth + 1
        end
...
function StoryDungeonMainScene:onClick(widget,_type)
    ...
    _parent = tolua.cast(_parent,"ImageView")
     local _head = _parent:getChildByName("ImageView_Head" .. 6-_parent:getTag())
    ...
    elseif _type == TOUCH_EVENT_ENDED then
        ...
        if _data then
            ...
            local child = _parent:getChildByName("Panel_WuJiangZhuan" .. 6-_parent:getTag())
            local passLabel = child:getChildByName("Label_Pass" .. 6-_parent:getTag())
            ...
            local labelCommingSoon = _parent:getChildByName("BitmapLabel_ComeBack" .. 6-_parent:getTag())
...
function StoryDungeonMainScene:_showChapterImg(cell, imgId, isOpen, index, id)
...
function StoryDungeonMainScene:_updateCell( list, index, cell)
    ...
    local _startIndex = #self._DungeonList - (num-index)*5
    ...
    for i=5,1,-1 do
        ...
        if data then
            if data.pic > 0 then
                ...
                -- 设置城池图标
                self:_showChapterImg(cell, data.copy_pic, true, i, data.id)
...
function StoryDungeonMainScene:countScrollSpace()
    local num = math.floor(#self._DungeonList%5)
    local lenth = 0
    if num == 0 then
        lenth = 0
    elseif num == 1 then
    ...
    if -lenth >1140- CCDirector:sharedDirector():getWinSize().height  and #self._DungeonList <= 5 then
        lenth = 0
    end
    return lenth
end
