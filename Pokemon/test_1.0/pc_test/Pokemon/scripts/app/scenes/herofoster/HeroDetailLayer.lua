...
function HeroDetailLayer:updateDetailWithKnightId( knightId )
    ...
    self:showWidgetByName("Label_shizhuang", false)
    ...
    self:showWidgetByName("Button_shizhuang", false)
    ...
    -- 化神
    self:showWidgetByName("Button_God", level >= 160)
    if not self._canGod then
    	...
    	-- end
        self:showTextWithLabel("Label_God", G_lang:get(""))
    end
