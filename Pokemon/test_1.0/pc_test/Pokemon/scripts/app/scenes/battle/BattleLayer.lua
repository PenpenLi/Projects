...
BattleLayer.BATTLE_SPEED_1 = 1
BattleLayer.BATTLE_SPEED_2 = 2
BattleLayer.BATTLE_SPEED_3 = 3
...
function BattleLayer:ctor(pack, eventHandler)
   if patchMe and patchMe("BattleLayer", self) then return end  
    ...
    self._doubleOptions = {1.5, 2.5, 3}
    self._doubleMaxFrames = {1.5, 2, 2.5}
    ...
    if not pack.double then
        ...
        self._double = params._double or BattleLayer.BATTLE_SPEED_1
        -- 看看是否到达开启等级，如果到达则直接跳过去
        params = storage.load(storage.rolePath("battle_double_level")) or {_doubleLevel=BattleLayer.BATTLE_SPEED_1}
        -- 直接找最高级别的
        if G_moduleUnlock:isModuleUnlock(FunctionLevelConst.BATTLE_RATE_3, true) and params._doubleLevel < BattleLayer.BATTLE_SPEED_3 then
            self._double = BattleLayer.BATTLE_SPEED_3
            storage.save(storage.rolePath("battle_double_level"), {_doubleLevel=BattleLayer.BATTLE_SPEED_3})
        elseif G_moduleUnlock:isModuleUnlock(FunctionLevelConst.BATTLE_RATE_2, true) and params._doubleLevel < BattleLayer.BATTLE_SPEED_2 then
            self._double = BattleLayer.BATTLE_SPEED_2
            storage.save(storage.rolePath("battle_double_level"), {_doubleLevel=BattleLayer.BATTLE_SPEED_2})
        end
        -- 记录等级
...
--    self:setScale(0.5)
    ...
    _loadCommonRes("battle/sp/sp_wzhq_juguang/sp_wzhq_juguang")
...
--        plistLoad.desc()
        ...
        TextureLoadFactory.loadTextureAsync(list, function(a,b)
            --print("async load finish", a,b)
        end)
        ]]
        --[[
        self:addEntryToSynchQueue(nil, function()
            print("addEntryToSynchQueue", #list)
            if isFinish then
                -- 是否全部完成
                ...
                if not totalFinish then
                    count = count + 1
                    isFinish = false
                    TextureLoadFactory.loadTextureAsync({list[count]}, function(a,b)
                        print("TextureLoadFactory.loadTextureAsync", a,b)
                        print("addEntryToSynchQueue finish", count)
                        isFinish = true
                    end)
                end
            end
            print("addEntryToSynchQueue.total", totalFinish)
            return totalFinish
        end)
        ]]
...
--    colorLoad.clear()
    ...
    local function _getSpPlist(spName)
        ...
        if (spJson.png and spJson.png ~= "") or checkResFile(spName..".png") then
...
--                        end
                    ...
                    for k, attacker in pairs(attackers) do
                        ...
                        if attack.identity == 2 then
                            if checkResFile("battle/action/"..playInfo.attack_action_id.."_r.json") then
...
--                        colorLoad.add(_getActionColor(attack_action_id))
                        ...
                        if attack.identity == 1 then
                            if checkResFile("battle/action/"..playInfo.defend_action_id.."_r.json") then
