...
function ActivityPageWine:ctor(...)
    ...
    if appstoreVersion or IS_HEXIE_VERSION then    
        ...
        -- self._caiwenjiEffect:setPosition(ccp(-55, -80))
        -- self._caiwenjiEffect:setScale(0.8)
        -- -- self:getPanelByName("Panel_caiwenji"):addNode(self._caiwenjiEffect) 
        -- bgImage:addNode(self._caiwenjiEffect) 
        
        local roleImg = ImageView:create()
        roleImg:loadTexture("effect/effect_caiwenji/effect_caiwenji.png")
        bgImage:addChild(roleImg)
        roleImg:setScale(0.8)
        roleImg:setPosition(ccp(20, 80))
    end
