...
function BagLayer.create(selectedIndex,packScene, tag)   
    return BagLayer.new("ui_layout/bag_Baglayer.json",selectedIndex,packScene,tag) 
end


function BagLayer:ctor(json,selectedIndex,packScene,tag,...)
...
function BagLayer:_initWidgets()
    ...
    self:registerBtnClickEvent("Button_backshop",function()
        --点击返回
...
function BagLayer:_checkedCallBack(btnName)
    if btnName == "CheckBox_item" then
        self:showWidgetByName("Button_backshop", false)
    ...
    elseif btnName == "CheckBox_juexing" then
        if self._tag == 1 and self._checkType == 2  then
            self:showWidgetByName("Button_backshop", true)
            self._tag = 0
        else
            self:showWidgetByName("Button_backshop", false)
        end
