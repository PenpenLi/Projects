...
function BagScene:ctor(selectedIndex,packScene, functionValue, tag, ...)
...
function BagScene:onSceneEnter()
    ...
    if self._bagLayer == nil then
        --第一次进入场景
        if self._tag == 1 then
            self._bagLayer= require("app.scenes.bag.BagLayer").create(self._selectedIndex,self._packScene, self._tag)
        else
            self._bagLayer= require("app.scenes.bag.BagLayer").create(self._selectedIndex,self._packScene, 0)
        end
