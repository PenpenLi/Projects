...
function StoryTalkItem:animationShow( func )
    ...
    if self._posIndex == 1 then -- 左
        self._talkItem:setPosition(ccp(_pos.x - size.width /3,_pos.y))
    ...
    arr:addObject(CCSpawn:createWithTwoActions(CCEaseExponentialOut:create(CCMoveTo:create(0.6,_pos)), CCEaseExponentialOut:create(CCScaleTo:create(0.6,1))) )
    ...
    self:runAction(CCMoveTo:create(0.1, ccp(posx, posy)))
