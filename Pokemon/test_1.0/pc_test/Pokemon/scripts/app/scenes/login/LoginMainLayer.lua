...
function LoginMainLayer:ctor( ... )
    ...
    self:getWidgetByName("ImageView_90"):addNode(effect)
...
function LoginMainLayer:_onClickEnterGame(  )
    ...
    track_event_start("8-click_enter_game")
...
function LoginMainLayer:onLayerEnter( ... )
    ...
    self:getCheckBoxByName("CheckBox_Protocol"):setTouchEnabled(false)
    self:getLabelByName("Label_protocal_tip"):setVisible(false)
...
function LoginMainLayer:onUpdateServer()
    ...
    if server ~= nil then
        ...
        require("app.scenes.login.ServerListLayer").updateServerLabel(self:getLabelByName("Label_weihu"), server.status)
