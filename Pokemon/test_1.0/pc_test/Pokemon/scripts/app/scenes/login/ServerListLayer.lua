...
function ServerListLayer.updateServerIcon(imageView, status)
    if status and status ~= 1  and status ~= 7 and status ~= 3 and status ~=4 then
...
function ServerListLayer.updateServerLabel(lab, status)
    --status: 游戏服类型，1-正常；2-新开；3-火爆；4-维护；5-停服；6-合服 ；7-即将开启
...
function ServerListLayer:_initViews()    
    ...
    local function updateCell(cell, data1, data2) 
        ...
        ServerListLayer.updateServerLabel(cell:getLabelByName("Label_weihu"), cell.serverData.status)
        ...
        if data2 then
            ...
            ServerListLayer.updateServerLabel(cell:getLabelByName("Label_weihu2"), cell.serverData2.status)
        else
