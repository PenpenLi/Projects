	...
	if not tListView then
		...
		if panel then
	        ...
	        tListView:setUpdateCellHandler(function(list, index, cell)
	        	local tTmplList = {}
	        	for i=1, 3 do
	        		table.insert(tTmplList, #tTmplList + 1, self._tSummonTmplList[nGroup][index * 3 + i])
	        	end
	        	cell:updateItem(tTmplList)
	        end)

	        local len = 0
	        if #self._tSummonTmplList[nGroup] % 3 ~= 0 then
	        	len = math.floor(#self._tSummonTmplList[nGroup] / 3) + 1
	        else
	        	len = #self._tSummonTmplList[nGroup] / 3
	        end
