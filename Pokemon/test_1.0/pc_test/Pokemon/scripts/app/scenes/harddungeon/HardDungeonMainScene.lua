...
function HardDungeonMainScene:ctor(josn, func, stageId, chapterId, pack, ...)
	...
	self:adapterLayerHeight(self._layer, nil, self._speedBar, -8, -100)
...
function HardDungeonMainScene:playWinAnimation()
	...
	if self._indexOfCell == 1 then
		...
		cityPanel = self.listview:getShowCellByIndex(1):getImageViewByName("ImageView_City" .. 5)
	elseif self._indexOfCell == 5 then
...
function HardDungeonMainScene:onClick(widget, _type)
	...
	local _city = _parent:getParent():getChildByName("ImageView_City" .. 6 - _parent:getTag())
	...
	if _type == TOUCH_EVENT_BEGAN then
		...
		-- _parent:getVirtualRenderer():setColor(FieldColor[6 - _parent:getTag()])
...
function HardDungeonMainScene:countScrollSpace()
	local num = math.floor(#self._chapterList % 5)
	local lenth = 0
	if num == 0 then
		lenth = 0
	elseif num == 1 then
	...
	if - lenth > 1140 - CCDirector:sharedDirector():getWinSize().height and #self._chapterList <= 5 then
		lenth = 0
	end
	return lenth
end
...
function HardDungeonMainScene:_countListLenth()
	local lenth = math.floor(#self._chapterList / 5)
	if #self._chapterList % 5 > 0 then
		lenth = lenth + 1
	end
...
function HardDungeonMainScene:playWayAnimation()
	local _index = 1
	self._timer = G_GlobalFunc.addTimer(0, function()
		-- 如果是第一则需要
		local lastCell = nil
		-- 当前开启的章节块 大于1
		...
		if self._indexOfCell == 1 and lastCell and lastCell:getImageViewByName("ImageView_LastRoad"):isVisible() == false then
            ...
            local cell = self.listview:getShowCellByIndex((self._indexOfCell == 5 and 1) or 0)
            if cell then
...
function HardDungeonMainScene:_updateCell(list, index, cell)
	...
	local _startIndex = #self._chapterList -(num - index) * 5
            ...
            if data then
                ...
                if data.map > 0 then
	                ...
	                if labelChapterBg and nameChapterBg and labelChapter and panelStar then
                		...
                		panelStar:setPositionY(_name:getPositionY() - 70)
	                end
                    ...
                    if self.secretShine then
                        ...
                        local _cellIndex =(tag == 5 and 1 or 0)
                    ...
                    self:_showChapterImg(cell, data.copy_pic, _isShowLight, i, data.id)
...
function HardDungeonMainScene:_showChapterImg(cell, imgId, isOpen, index, id)
    ...
    local _fieldImage = cell:getImageViewByName("ImageView_Field" .. index)
	-- 创建地图
	-- _cityImage:loadTexture(G_Path.getCityIcon(imgId))
    _fieldImage:loadTexture(G_Path.getFieldIcon(imgId))
