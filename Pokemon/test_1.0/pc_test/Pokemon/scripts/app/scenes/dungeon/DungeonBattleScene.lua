...
function DungeonBattleScene:onSceneEnter(...)
    ...
    if IS_IPHON_ANDROID then 
        if self._stageId ~= nil then 
            TDGAMission:onBegin("关卡[" .. self._stageId .. "]") 
            print("统计->开始关卡:" .. self._stageId)
        end
    end
end
...
function DungeonBattleScene:finishBattleCallBack(isWin)
    ...
    self._isWin = isWin
    if IS_IPHON_ANDROID then 
        if self._isWin ~= nil and self._stageId ~= nil then 
            if self._isWin == true then 
                TDGAMission:onCompleted("关卡[" .. self._stageId .. "]") 
                print("统计->通过关卡成功:" .. self._stageId)
            else 
                TDGAMission:onFailed("关卡[" .. self._stageId .. "]", "无") 
                print("统计->通过关卡失败:" .. self._stageId)
            end
        end
    end
