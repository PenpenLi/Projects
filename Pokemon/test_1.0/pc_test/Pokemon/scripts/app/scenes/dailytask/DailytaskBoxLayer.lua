...
function DailytaskBoxLayer:setStarBoxStatus(gateStar,isGet,_totalStar)
    ...
    function setRichText()
        ...
        if _descLabel then 
            local str = "<content>" .. self:_appendRichText(G_lang:get("LANG_DAILYTASK_DES1"), '1292212')
            str = str .. self:_appendRichText(gateStar .. G_lang:get("LANG_DAILYTASK_DES2"), '1292212')
            str = str .. self:_appendRichText(G_lang:get("LANG_DAILYTASK_DES3"), '1292212') .. "</content>"
            _richText:appendXmlContent(str)
            _richText:reloadData()
        end
    end
