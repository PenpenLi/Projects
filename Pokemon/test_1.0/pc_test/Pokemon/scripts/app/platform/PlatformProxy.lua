...
function PlatformProxy:loginGame()
    track_event_start("11-login_game")
...
function PlatformProxy:sendLoginGame()
    ...
    local server = self:getLoginServer()
    print("sendLoginGame:", channel_id, device_id, server.id, self._token)
    ...
    if IS_TASTE_VERSION == false and IS_IPHON_ANDROID then 
        TDGAAccount:setGameServer(tostring(server.id))
        print("统计->登录服务器:serverid:" .. server.id)
    end
end
...
function PlatformProxy:_onLoginSuccess()
    ...
    --统计
    if IS_IPHON_ANDROID == true then
        TDGAAccount:setAccount(self._platform_uid .. "+" .. server.id)
        TDGAAccount:setAccountType(TDGAAccount.kAccountRegistered)
        print("统计->登录账号:uid:" .. self._platform_uid .. "+" .. server.id .. ", accountType:" ..TDGAAccount.kAccountRegistered)
    end
...
function PlatformProxy:_onBlackCardUser( ... )
    MessageBoxEx.showOkMessage(nil, G_lang:get("LANG_BLACK_CARD_USER"), true, function ( ... )
        if G_NativeProxy.openURL then
            ...
            G_NativeProxy.openURL("http://localhost/djss")
        end
    end)
end
...
function PlatformProxy:setPlatformId(id)
    self._platform_uid = id
    print("PlatformProxy:setPlatformId") 
    uf_eventManager:dispatchEvent(G_EVENTMSGID.EVENT_UPDATE_UID, nil, true) 
end

function PlatformProxy:setUid(uid, fromServer)
    self._uid = uid
...
function PlatformProxy:getSdkUid()
    return self._sdk_uid
end

function PlatformProxy:getUid()
    return self._uid
end
...
function PlatformProxy:isAccountLogin()
    return self._platform_uid ~= ""
end
