...
function check_json_format(str)
    local json = require "framework.json"
    str = json.decode(str)
    if str == nil then
        return false
...
function checkResFile(fileName)
    local fileSrc = fileName
    if target ~= kTargetWindows then
        fileName =  FuncHelperUtil:getResFileName(fileName)
    end
    local fileUtils = CCFileUtils:sharedFileUtils()
    local r = fileUtils:isFileExist(fileUtils:fullPathForFilename(fileName))
    --print("checkResFile", fileSrc, fileName, r)
    return r
end
