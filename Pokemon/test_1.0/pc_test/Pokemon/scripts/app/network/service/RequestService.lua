...
function RequestService:addRequest(id, msg)
    ...
    for i,request in ipairs(self._requestList) do 
        --print("  ", request._id, request._responseId)
    end
    ...
    for i,request in ipairs(self._requestList) do 
        if request:isSent() then
            if request:checkResponse(msgId) then
                local useTime =  FuncHelperUtil:getTickCount() - request._sendTime
                table.remove(self._requestList, i , 1)
                --print("recv<<<", msgId, useTime, request._id)
                break
            end
        end
    end
...
function RequestService:onDisconnected() 
    print("RequestService:onDisconnected")
...
function RequestService:_onTimer() 
    ...
    --[[print("+++++++++++++++++++++++++++++++++++++++++++++")
    for k,v in ipairs(self._requestList) do
        print(v:dumpRequestInfo())
    end
    print("+++++++++++++++++++++++++++++++++++++++++++++")]]
    --check timeout
    local timeout = false
