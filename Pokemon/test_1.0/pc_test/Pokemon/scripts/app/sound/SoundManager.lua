...
function SoundManager:playSound(soundName,isLoop)
    ...
    local soundName2 = self:getSoundFile(soundName)

    local handler = audio.playSound(soundName2,isLoop)
...
function SoundManager:stopSound(soundName)
    if self.battleSounds[soundName] and self.soundList[soundName] then 
...
function SoundManager:preloadSound(soundName)
    soundName = self:getSoundFile(soundName)
