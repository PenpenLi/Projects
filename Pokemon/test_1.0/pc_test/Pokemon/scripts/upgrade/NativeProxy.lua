...
function NativeProxy.new()
    ...
    NativeProxy.channelName = ""
    NativeProxy.channelId = "0"
    NativeProxy.subChannelId = "0"
	...
	elseif target == kTargetAndroid then
	    ...
	    NativeProxy.channelName = tostring(NativeProxy.native_call("getMeta", {{metaKey="IsChannelName"}}, "string"))
	    NativeProxy.channelId = tostring(NativeProxy.native_call("getMeta", {{metaKey="IsChannelID"}}, "string"))
...
function NativeProxy.native_call(func, param, returnType)
	...
	elseif NativeProxy.platform == "android" then
		--android
		local SDK_CLASS_NAME = "com/zywl/platform/NativeProxy"
...
function NativeProxy.hasNetwork()
    if NativeProxy.platform == "windows" or NativeProxy.platform =="mac" then
        return true
    end
