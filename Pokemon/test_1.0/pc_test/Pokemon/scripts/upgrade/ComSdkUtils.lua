...
function ComSdkUtils.call(func, param, returnType)
    ...
    elseif G_NativeProxy.platform == "android" then
        --android
        local SDK_CLASS_NAME = "com/zywl/platform/NativeComSdkProxy"
...
function ComSdkUtils.updateOpId(  )
    ...
    if ext ~= nil and ext ~= "" then
        ...
        if params and type(params) == "table" and #params >= 2 then
            opId = params[1]
            sdkId = params[2]
            --gameId = params[2]
            --gameOpId = params[3]
...
function ComSdkUtils.getOpId()
    print("ComSdkUtils.getOpId", opId)
    if opId ~= "" then
        return opId
    end
